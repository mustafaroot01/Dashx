...
import {
    Plus, Loader2, Trash2, Pencil, Calendar, MoreVertical
} from "lucide-react"
import {
    DropdownMenu,
    DropdownMenuContent,
...
export default function StudyTypesPage() {
    const { data: studyTypes, mutate: mutateTypes, isLoading } = useSWR<any[]>('/study-types', fetcher)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditMode && editingId) {
                await axiosClient.put(`/study-types/${editingId}`, { name })
            } else {
                await axiosClient.post('/study-types', { name })
            }
            setIsDialog(false)
            mutateTypes()
    ...
    const executeDelete = async () => {
        if (!deleteId) return
        try {
            await axiosClient.delete(`/study-types/${deleteId}`)
            mutateTypes()
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 bg-white/10 rounded-lg backdrop-blur-sm border border-white/10">
                            <Calendar className="h-6 w-6 text-[var(--sidebar-primary)]" />
                        </div>
                    ...
                    <p className="text-[var(--sidebar-foreground)]/80 text-sm max-w-xl">
                        إدارة أنواع الدراسة (صباحي، مسائي، متميزين ...).
                    </p>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {studyTypes?.map(study => (
                        <Card key={study.id} className="group border-t-[4px] border-t-[var(--primary)] shadow-sm hover:shadow-xl transition-all duration-300 bg-card">
                            <CardHeader className="flex flex-row items-center justify-between pb-2 bg-[var(--muted)]/30 border-b border-[var(--border)] px-5 py-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <CardTitle className="text-lg font-bold text-[var(--foreground)] tracking-tight">{study.name}</CardTitle>
                                </div>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="border-[var(--border)]">
                                        <DropdownMenuItem onClick={() => openEdit(study)} className="focus:bg-[var(--muted)]"><Pencil className="mr-2 h-3 w-3" /> تعديل</DropdownMenuItem>
                                        <DropdownMenuItem className="text-destructive focus:bg-destructive/10" onClick={() => setDeleteId(study.id)}><Trash2 className="mr-2 h-3 w-3" /> حذف</DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </CardHeader>
                            <CardContent className="pt-6 px-5">
                                <p className="text-sm text-muted-foreground">تاريخ الإنشاء: {new Date(study.created_at).toLocaleDateString('ar-IQ')}</p>
                            </CardContent>
                        </Card>
                    ))}
                </div>
